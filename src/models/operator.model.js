...
function postProducts(products, id_delivery){
    ...
    products.forEach(product => {
        pool.query('INSERT INTO delivery_products (product_id_product, quantity, delivery_id_delivery) VALUES (?, ?, ?)', [product.product_id, product.quantity, delivery])
    });
    return pool.query('SELECT * FROM delivery_produts WHERE delivery_id_delivery = ?', [delivery])
}
function updateDeliveryById(delivery_info, id_delivery){
...
module.exports = {
    selectAll, selectById, postDelivery, postProducts, updateDeliveryById, updateProductsById, selectProductByDelivery
}